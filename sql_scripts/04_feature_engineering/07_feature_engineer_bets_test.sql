-- Create or replace view for testing dataset features by joining bets and manual assessments

CREATE OR REPLACE VIEW customer_analysis_se.vw_combined_bets_test_features AS
SELECT 
    b.customer_id,
    b.timestamp,
    b.winning_cash_num,
    ...
    m.date
FROM 
    customer_analysis_se.combined_bets_test b  -- Note the change to the test dataset
INNER JOIN 
    customer_analysis_se.manual_assessments_test m ON b.customer_id = m.customer_id  -- Joining with test assessments
WHERE 
    b.timestamp <= m.date;

CREATE TABLE customer_analysis_se.combined_bets_test_features_aggregated AS
WITH quantiles AS (
    SELECT
        customer_id,
        date AS label_date,
    ...
    FROM customer_analysis_se.vw_combined_bets_test_features
...
FROM 
    customer_analysis_se.vw_combined_bets_test_features bf
JOIN 
