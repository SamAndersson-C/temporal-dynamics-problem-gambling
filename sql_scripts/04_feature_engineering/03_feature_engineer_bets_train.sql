-- Create or replace view for training dataset features by joining bets and manual assessments
...
SELECT 
    ...
    m.date
FROM 
    customer_analysis_se.combined_bets_train b
INNER JOIN 
    customer_analysis_se.manual_assessments_train m ON b.customer_id = m.customer_id
WHERE 
    b.timestamp <= m.date;

CREATE TABLE customer_analysis_se.combined_bets_features_aggregated AS
WITH quantiles AS (
    SELECT
        customer_id,
        date AS label_date,
    ...
    FROM customer_analysis_se.vw_combined_bets_features
