...
FROM 
    customer_analysis_se.combined_payments_test p
INNER JOIN 
    customer_analysis_se.manual_assessments_test m ON p.customer_id = m.customer_id
WHERE 
    p.timestamp_max <= m.date;


CREATE TABLE customer_analysis_se.combined_payments_test_features_aggregated AS
