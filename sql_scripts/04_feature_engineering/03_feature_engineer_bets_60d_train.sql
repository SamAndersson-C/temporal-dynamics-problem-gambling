...
SELECT 
    ...
    m.date
FROM 
    customer_analysis_se.bets_data_excl_60d b
INNER JOIN 
    customer_analysis_se.manual_assessments_train m ON b.customer_id = m.customer_id
WHERE 
    b.timestamp <= m.date;

DROP TABLE IF EXISTS temp_combined_bets_features;
...
-- Aggregate betting features over 60 days, including quantiles and averages for various betting metrics

CREATE TABLE customer_analysis_se.combined_bets_features_aggregated_60d AS
WITH quantiles AS (
    SELECT
        customer_id,
        label_date,
